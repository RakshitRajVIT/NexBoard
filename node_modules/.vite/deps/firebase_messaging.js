import { A as validateIndexedDBOpenable, C as openDB, D as areCookiesEnabled, E as FirebaseError, O as getModularInstance, S as deleteDB, T as ErrorFactory, b as registerVersion, c as _getProvider, d as _registerComponent, h as getApp, k as isIndexedDBAvailable, w as Component } from "./index.esm2017-DxzxTqoC.js";
//#region node_modules/@firebase/installations/dist/esm/index.esm2017.js
var name$1 = "@firebase/installations";
var version$1 = "0.6.18";
/**
* @license
...
function getDbPromise$1() {
	if (!dbPromise$1) dbPromise$1 = openDB(DATABASE_NAME$1, DATABASE_VERSION$1, { upgrade: (db, oldVersion) => {
		switch (oldVersion) {
			case 0: db.createObjectStore(OBJECT_STORE_NAME$1);
		}
	} });
	return dbPromise$1;
}
...
async function set(appConfig, value) {
	const key = getKey$1(appConfig);
	const tx = (await getDbPromise$1()).transaction(OBJECT_STORE_NAME$1, "readwrite");
...
function getDbPromise() {
	if (!dbPromise) dbPromise = openDB(DATABASE_NAME, DATABASE_VERSION, { upgrade: (upgradeDb, oldVersion) => {
		switch (oldVersion) {
			case 0: upgradeDb.createObjectStore(OBJECT_STORE_NAME);
		}
	} });
	return dbPromise;
}
...
async function dbGet(firebaseDependencies) {
	...
	const tokenDetails = await (await getDbPromise()).transaction(OBJECT_STORE_NAME).objectStore(OBJECT_STORE_NAME).get(key);
...
async function requestGetToken(firebaseDependencies, subscriptionOptions) {
	...
	try {
		...
		throw ERROR_FACTORY.create("token-subscribe-failed", { errorInfo: err === null || err === void 0 ? void 0 : err.toString() });
	}
...
async function requestUpdateToken(firebaseDependencies, tokenDetails) {
	...
	try {
		...
		throw ERROR_FACTORY.create("token-update-failed", { errorInfo: err === null || err === void 0 ? void 0 : err.toString() });
	}
	if (responseData.error) {
		const message = responseData.error.message;
...
async function requestDeleteToken(firebaseDependencies, token) {
	...
	try {
		...
		throw ERROR_FACTORY.create("token-unsubscribe-failed", { errorInfo: err === null || err === void 0 ? void 0 : err.toString() });
	}
}
...
function propagateFcmOptions(payload, messagePayloadInternal) {
	var _a, _b, _c, _d, _e;
	if (!messagePayloadInternal.fcmOptions && !((_a = messagePayloadInternal.notification) === null || _a === void 0 ? void 0 : _a.click_action)) return;
	payload.fcmOptions = {};
...
async function registerDefaultSw(messaging) {
	try {
		...
		throw ERROR_FACTORY.create("failed-service-worker-registration", { browserErrorMessage: e === null || e === void 0 ? void 0 : e.message });
	}
}
/**
...
var WindowMessagingInternalFactory = (container) => {
	...
	return { getToken: (options) => getToken$1(messaging, options) };
};
